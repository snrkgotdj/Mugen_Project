	...
	while (GET_SINGLE(CPacketQueue)->Pop(&tPacket))
	{
		...
		case PH_CHARACTORSEL:
		{
			...
			if (cPlayer == 1)
			{
				...
				GET_SINGLE(CNetworkManager)->SendPacket(PH_CHARACTORSEL, (char*)cPlayerName, wcslen(cPlayerName) * 2, false);
			}
			...
			else if (cPlayer == 2)
			{
				...
				GET_SINGLE(CNetworkManager)->SendPacket(PH_CHARACTORSEL, (char*)cPlayerName, wcslen(cPlayerName) * 2, true);
			}
