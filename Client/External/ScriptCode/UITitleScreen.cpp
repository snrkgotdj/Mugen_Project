...
{
	CSoundLoadMgr::GetInst()->Awake();
	
}

void CUITitleScreen::Start()
{
	CSoundLoadMgr::GetInst()->Start();
}
...
void CUITitleScreen::Update()
{
	if (m_pSound == NULL)
	{
		CSoundLoadMgr::GetInst()->LoadSoundFolder(L"00Common");
		CSoundLoadMgr::GetInst()->LoadSoundFolder(L"01BGM");
		m_pSound = CResManager::Manager().FindSound(L"MainMenu.wav");
		m_pSound->Play(-1);
	...
	if (!m_bEnterPushed)
	{
		for (iter; iter != iterEnd; ++iter)
		{
			...
			if ((*iter)->GetTagName() == L"EnterKey")
			{
				if ((*iter)->GetAnimatorEX()->GetCurAnim()->GetCurIndex() != 0)
					(*iter)->GetTransform()->SetLocalScale(Vec3Zero);
				else
