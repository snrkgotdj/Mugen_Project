...
void CPlayerManager::SetName(const wstring & _strName, char _cPlayer)
{
	if (1 == _cPlayer)
	{
		m_b1P = false;
...
void CPlayerManager::Update()
{
	...
	{
		if (str1PClip == L"Idle"||str1PClip==L"Crouch"|| str1PClip==L"앉기")
			((CCharactorScript*)m_pGameObject_1P->GetFirstScript())->SetDir(DIR_RIGHT);
		if (str2PClip == L"Idle" || str2PClip == L"Crouch" || str2PClip == L"앉기")
