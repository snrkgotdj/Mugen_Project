...
void CPlayerScKyo::Awake()
{
	...
	CEffectManager::GetInst()->LoadEffect(L"Kyo");
...
void CPlayerScKyo::Start()
{
	...
	//CreateShadow();
	//((CShadowScript*)(m_pShadow->GetFirstScript()))->init(GetGameObject(), 0.06f, 0.18f);
...
int CPlayerScKyo::CommandDecode_Size4()
{
	...
	else if (m_eState == KYO_TYPE401_TSUMIYOMI)
	{
		if (m_vecCmd[iSize - 4] == m_tKey.Left
			...
			&&m_vecCmd[iSize - 2] == m_tKey.Left
			&&m_vecCmd[iSize - 1] == m_tKey.Right
			&& (INPUT_PLAYER(m_tKey.X, STATE_DOWN)))
		{
...
int CPlayerScKyo::CommandDecode_Size2()
{
	...
	{
		if (m_vecCmd[iSize - 2] == m_tKey.Down&&m_vecCmd[iSize - 1] == m_tKey.Right
			&& (INPUT_PLAYER(m_tKey.Z, STATE_DOWN)))
		{
...
void CPlayerScKyo::TypeU108_Orochinagi()
{
	...
	else if (7 == m_iBeforeIdx)
	{
		if (!m_bEffect)
		{
			...
			pSound->Play(1);
			CEffectManager::GetInst()->ActiveEffect(L"Kyo_108식_2", m_vecPos+Vec3(150.f*m_iDir,0.f,0.f),Vec3(-400.f*m_iDir,400.f,1.f));
			CEffectManager::GetInst()->ActiveEffect(L"Kyo_108식_3", m_vecPos, Vec3(-400.f*m_iDir, 400.f, 1.f));
			m_bEffect = true;
		}
	}
...
void CPlayerScKyo::Type114_Aragami()
{
	...
	if (0 == iRand)
	{
		SoundOutput(L"Kyo_Aragami1.wav", 2);
		SoundOutput(L"Kyo_Aragami3.wav",15);
	}
	else if (1 == iRand)
	{
		SoundOutput(L"Kyo_Aragami2.wav", 2);
		SoundOutput(L"Kyo_Aragami4.wav",15);
	}
...
void CPlayerScKyo::Type115_Dokugami()
{
	...
	if (m_iBeforeIdx != 7)
		m_bSound = false;
	SoundOutput(L"Kyo_Dokugami.wav", 7);
...
void CPlayerScKyo::Type127_Yanosabi()
{
	int iRand = getRandomNumber_int(0, 1);
	if (0 == iRand)
		SoundOutput(L"Kyo_Migiri1.wav");
	else if (1 == iRand)
		SoundOutput(L"Kyo_Migiri2.wav");
...
void CPlayerScKyo::Type128_Konokizu()
{
	int iRand = getRandomNumber_int(0, 1);
	if (0 == iRand)
		SoundOutput(L"Kyo_Konokizu1.wav");
	else if (1 == iRand)
		SoundOutput(L"Kyo_Konokizu2.wav");
...
void CPlayerScKyo::Type182()
{
	...
	if (!m_bEffect)
	{
		...
		if (15 == m_iBeforeIdx)
		{
			...
			CEffectManager::GetInst()->FollowPos(L"Kyo_182식_2", m_vecPos+Vec3(0.f,25.f,0.f), m_iDir - 1);
		}
